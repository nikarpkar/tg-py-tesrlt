            ...
            with open(download_path, "wb") as file:
                ...
                while True:
                    ...
                    if time.time() - last_update_time >= 10:
                        last_update_time = time.time()
                        await update_progress_message(message, current, total, speed, start_time)

async def update_progress_message(message, current, total, speed, start_time):
...
async def upload_file(client, file_path, chat_id, message, start_time):
...
async def process_file(client, chat_id, url, download_path, message):
    ...
    await upload_file(client, download_path, chat_id, message, time.time())
